...
def get_title(article_Code):
    ...
    # titleNum += 1
    response_of_article = requests.get(
...
def get_ALL(article_Code, fourmType):
    ...
    try:
        _extracted_from_get_ALL_18(bs_pages, fourmType)
    except ValueError:
        pass
    except:
        magnet_error_List.append(
            bs_pages.find("span", attrs={"id": "thread_subject"}).get_text()
        )
        article_Code_error_List.append(article_Code)

    if magnet_error_List:
        for title, article_Code_error in zip(
            magnet_error_List, article_Code_error_List
        ):
            print(f"\n{title} ")
            print(
                f"    無 magnet 請開啟連結: "
...
def _extracted_from_get_ALL_18(bs_pages, fourmType):
    ...
    title_List.append(title)
    magnet_List.append(
        bs_pages.find("div", "blockcode").get_text().removesuffix("复制代码")
    )
