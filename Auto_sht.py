...
def check_update(current_version):
    ...
    latest_version = (
        soup.find("div", "f1 flex-auto min-width-0 text-normal").get_text().strip()
    )
...
def extract():
    ...
    print(
        ...
        + str(today)
        + " "
        + str(fourmList_Chinese[fourmChoose - 1])
        + " 區的挑選作業:"
    )
    ...
    if fourmtype == "有碼":
        print(
            f"[*]一共排除了 {len(today_list[fourmChoose-1]) - len(title_List)} 篇素人文章，並保留了 {len(title_List)} 篇文章"
        )
    ...
    for title in title_List:
        ...
        if if_save == "":
            ...
            if if_save == "":
                ...
                # Case: Do not save
                # """
                workSpace.title_magnet[
                    title_List_history[Num - 1]
                ] += "DO_NOT_SAVE"  # magnet .. ... .DO_NOT_SAVE
            elif if_save == "exit":
                # """
                # Case: Exit - Stage I
                # """
            ...
            if check_if_save[-11:] == "DO_NOT_SAVE":
                ...
                workSpace.title_magnet[title_List_history[Num - 1]] = mag_temp.replace(
                    "DO_NOT_SAVE", ""
                )
    ...
    magnet_choosen = [
        x for x in workSpace.title_magnet.values() if x[-11:] != "DO_NOT_SAVE"
    ]
